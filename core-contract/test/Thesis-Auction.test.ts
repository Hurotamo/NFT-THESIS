...
describe("ThesisAuction Contract", function () {
  ...
  it("Should allow buying NFT with sufficient ETH", async function () {
    ...
    // Note: The contract requires buyers to be in the _minters mapping, but no public method to add minters.
    // This test assumes the buyer is allowed to buy for demonstration purposes.
  ...
  it("Should end auction and refund non-winners", async function () {
    const currentSupply = await thesisNFT.totalSupply();
    if (Number(currentSupply) < maxSupply) {
      await thesisNFT.mint(maxSupply - Number(currentSupply), { value: price * BigInt(maxSupply - Number(currentSupply)) });
    }
    ...
    // Simulate addr1 buying NFT
    await thesisAuction.connect(addr1).buyNFT(1, { value: auctionPrice });

    // Simulate addr2 buying NFT (should fail because NFT is already sold)
    await expect(thesisAuction.connect(addr2).buyNFT(1, { value: auctionPrice })).to.be.reverted;

    // End auction and refund non-winners
    await expect(thesisAuction.endAuction(1))
      .to.emit(thesisAuction, "AuctionEnded")
      .withArgs(addr1.address, auctionPrice);
  });
});
